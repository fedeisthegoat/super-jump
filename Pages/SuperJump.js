...
export default function MarioGame() {
  ...
  const drawPlayer = (ctx, player) => {
    ...
    if (isMoving) {
      ...
      ctx.beginPath();
      ctx.ellipse(x + 8, y + h - 3, 8, 5, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(x + w - 8, y + h - 3, 8, 5, 0, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    if (player.hasShield) {
      ...
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.arc(x + w / 2 + bodyLean, y + h / 2, 28, 0, Math.PI * 2);
      ctx.stroke();
      
      ctx.strokeStyle = `rgba(0, 200, 255, ${shimmer * 0.5})`;
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(x + w / 2 + bodyLean, y + h / 2, 32, 0, Math.PI * 2);
      ctx.stroke();
    }
  ...
  const gameLoop = () => {
    ...
    if (currentLevel === 1) {
      // Kleine Häuser im Hintergrund - auf dem Boden (y = 550 - 60 (house height) = 490)
      drawHouse(ctx, 80, 490, '#FFE4E1', '#DC143C'); // Rosa Haus mit rotem Dach
      drawHouse(ctx, 150, 490, '#F0E68C', '#DAA520'); // Gelbes Haus mit goldenem Dach
      drawHouse(ctx, 350, 490, '#E6E6FA', '#9370DB'); // Lavendel Haus mit lila Dach
      drawHouse(ctx, 420, 490, '#FFDAB9', '#D2691E'); // Pfirsich Haus mit braunem Dach
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-60 rounded-lg">
                    <div className="text-center space-y-6 p-8">
                      ...
                      <div className="text-white space-y-2 mb-6">
                        ...
                        <p className="text-green-300 mt-2">🏆 4 Level zu meistern!</p>
